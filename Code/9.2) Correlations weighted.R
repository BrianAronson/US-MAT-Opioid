...
#3) create weights based on all info prior to 2012
    ...
    #c) make weighted vars
        ...
        cor(df.4$no_has_MAT,df.4$weight,use="complete")
        cor(df.4$no_has_MAT,df.4$w.MAT.perperson,use="complete")
        cor(df.4$no_has_MAT,df.4$MAT_pill_total,use="complete")
        cor(df.4$no_has_MAT/df.4$weight,df.4$w.MAT.perperson,use="complete")
        
        cor(df.2$no_has_MAT,df.2$pillsperperson,use="complete")
        cor(df.2$no_has_MAT,df.2$w.pill.perperson,use="complete")
        cor(df.2$no_has_MAT,df.2$ins.pillsperperson,use="complete")
        
        
        
        cor(df$w.opioid_pill_total,df$pills,use="complete")
        cor(df$w.opioid_pill_total/df$ind.pop,df$pills/df$ind.pop,use="complete")
        
        cor(df.2$w.pill.perperson,df.2$pillsperperson,use="complete")
        
        cor(df.2$MAT_pill_total,df.2$w.MAT,use="complete")
        cor(df.2$MAT_pill_total,df.2$no_has_MAT,use="complete")
        cor(df.2$w.MAT,df.2$no_has_MAT,use="complete")
        cor(df.2$w.MAT.perperson,df.2$no_has_MAT,use="complete")
        cor(df.2$w.MAT.perperson,df.2$w.MAT,use="complete")
...
#3) aggregate variablesacross years of interest
    ...
    #b) double check that pill sales are relatively consistant over time
        cor(df.2$ins.pillsperperson[df.2$county %in% df.4$county],df.4$ins.pillsperperson[df.4$county %in% df.2$county],use="complete")
            #yes. this is about as good as you can hope
    ...
    #b) graph with text, prettier labels. etc.
        ...
        #ii) graph
            ggheatmap2<-ggheatmap +
              ...
              ggtitle("Correlations among Opioid and MAT pill sales",subtitle="Pills per person (2016-2018)")+
        ...
        #iii)save
              ggsave(
                  tsave1 <-
                      paste(
                          fig.dir,
                          i,"Correlations among Opioid and MAT pill sales2.png",
                          sep = ""
                      ),
                  ggheatmap2,
                  width = 7,
